...
namespace AuthFlow.Application.Repositories.Interface.Repository
{
    ...
    public interface IRepositoryOperations<T> where T : class, IEntity
    {
        ...
        Task<OperationResult_REVIEWED<T>> GetUserById(int id);

        // The GetAllByFilter method returns all entities of type T that satisfy the provided predicate.
        // It takes a lambda expression that represents a condition the entities should meet.
        Task<OperationResult_REVIEWED<IQueryable<T>>> GetAllByFilter(Expression<Func<T, bool>> predicate);
        ...
        Task<OperationResult_REVIEWED<IQueryable<T>>> GetPageByFilter(int pageNumber, int pageSize, string filter);

        // The GetCountByFilter method returns the number of entities that satisfy the provided filter string.
        Task<OperationResult_REVIEWED<int>> GetCountByFilter(string filter);

        // The Add method adds a new entity to the repository and returns the ID of the added entity.
        Task<OperationResult_REVIEWED<int>> Add(T entity);
        ...
        Task<OperationResult_REVIEWED<bool>> Modified(T entity);
        ...
        Task<OperationResult_REVIEWED<bool>> Remove(int id);
        ...
        Task<OperationResult_REVIEWED<bool>> Deactivate(int id);
        ...
        Task<OperationResult_REVIEWED<bool>> Activate(int id);

        Task<OperationResult_REVIEWED<int>> GetCountFilter(string filter);
    }
}
